    ...
    <div className="flex flex-col gap-[18px]">
      <div className="w-1/2">
        <BigMetricCard
          ...
          delta={weekActivities.length > 0 ? `${weekActivities.length} activiteiten deze week` : '–'}
          Icon={TrendingUp}
        />
      </div>
...
export function CyclingSection({ activities }: { activities: Activity[] }) {
  const sevenDaysAgo = new Date(Date.now() - 7 * 86400000).toISOString()
  const weekRides = activities.filter(a => a.sport_type?.toLowerCase().includes('ride') && a.start_date >= sevenDaysAgo)
    ...
    <div className="flex flex-col gap-6">
      ...
      <div>
        ...
        <span className="text-[15px] font-medium text-cyan-400">
          {weekKm > 0 ? `${weekRides.length} rides` : '–'}
        </span>
      </div>
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div>
        ...
        <span className="text-[15px] font-medium text-orange-400">–</span>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <SmallCard title="Sessions" value={`${weekHevy.length || '–'}`} detail={weekHevy.length > 0 ? 'of 4 planned' : '–'} Icon={Timer} tint="text-blue-400" />
      </div>
