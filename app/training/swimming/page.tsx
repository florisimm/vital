...
import { SwimmingSection, computeRecoveryDetail } from '../sections'
...
export default function SwimmingPage() {
  ...
  const savedTargets = (data as any)?.zoneTargets?.swimming ?? null
  ...
  async function handleSaveTargets(newTargets: ZoneTargets) {
    ...
    await supabase.from('user_settings')
      .update({ training_zone_targets: { ...(data as any)?.zoneTargets, swimming: newTargets } })
      .eq('user_id', user.id)
    mutate('training')
  }
  ...
  return (
    ...
    >
      <SwimmingSection activities={activities} />
      <SportPlanCard
        sport="swimming"
        freq={freq}
        injured={injured}
