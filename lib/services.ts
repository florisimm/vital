...
export type Services = {
  strava: boolean; hevy: boolean; google: boolean; fitbit: boolean
  // Google Health (fitbit_tokens) is connected but its refresh token died —
  // the server-side sync set needs_reconnect=true and the user must re-auth.
...
export async function fetchServices(): Promise<Services> {
  ...
  if (!user) return { strava: false, hevy: false, google: false, fitbit: false, fitbitNeedsReconnect: false }
